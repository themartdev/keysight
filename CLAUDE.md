...
- `timing/` `MonotonicClock` and `RunTimeline`, every scheduled instant of one run: segment k
  ...
  last segment.
- `run/` `RunConfig` (tempo, metronome mode, visibility mode, lookahead, segment count),
  `VisibilityPolicy` with the Flash, Read ahead and Open score presets, `runMask` (the policy at
  a beat as a `Mask`), `Segment` and `runScore` (segments chained into one score with a resting
  measure 0), `RunContext`, `RunState`, `RunEvent`, the pure `RunMachine` reducer,
  `RunController` (the coroutine that drives it), `RunHistory` and `RunRecord`.
...
- `evaluation/` `PlayedNotes` (MIDI to notes on the run's beat line), `NoteAlignment` (edit
  distance over pitch and onset), `BeatPhase` (the player's lean on the click, bounded),
  `RhythmAnalysis` (timing, tempo ratio, pauses, continuity), `PerformanceEvaluator` with
  `EVALUATOR_VERSION`. It still evaluates a whole run at its end; the incremental evaluator is
  the second half of Round 6.
...
- `data/` Room: entities, DAOs, `KeySightDatabase` (schema version 2, `Migrations.kt`),
  `RoomRunHistory`, and the pure mappers. Runs are stored one per row on the attempt tables
  until schema version 3 brings runs and segments: `configJson` holds a `RunConfig`, the
  exercise column the segments' ids, and the preview column the lookahead, infinite when
  unbounded.
...
Not built yet: the second half of Round 6 (incremental evaluation on a trailing window, the
running beat phase, marks behind the cursor, schema version 3 with runs and segments, the run
summary of the plan, open-ended runs), then the generator, difficulty adaptation and session
summaries; the plan's round ladder covers them in order.
...
- Raw MIDI is never discarded or overwritten: `midi_events` rows are the three raw bytes and a
  ...
  Evaluations are keyed by `(attemptId, evaluatorVersion)`; bump `EVALUATOR_VERSION` whenever
  a judgement would change.
- `RunMachine` is a pure reducer. `RunController` wakes up only at
  `RunMachine.nextDeadlineNanos` or when that deadline moves (a stop), and whether a MIDI event
  is captured is decided by its own timestamp, not by the state at delivery. All state changes
...
- Beat 0 is when the first click reaches the listener: `AudioTrackMetronome` reads
  ...
  the page window and the beat dots all come from that one number, and none of them tick.
